        ...
        <div className="flex flex-wrap justify-center gap-4">
          {filters.map((filter) => (
            <button
              ...
              className={`relative px-6 py-3 rounded-full font-semibold transition-all cursor-pointer whitespace-nowrap flex items-center space-x-2 overflow-hidden ${
                activeFilter === filter.id
                  ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg'
                  : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200'
              }`}
            >
              {/* Icon behind the emoji */}
              <i className="ri-circle-fill absolute -left-3 lg:top-2 top-1 text-[#33EFA0] z-0 text-3xl lg:text-4xl"></i>
              <span className="z-10">{filter.emoji}</span>
              <span className="z-10">{filter.label}</span>
            </button>
          ))}
        </div>
      </div>
